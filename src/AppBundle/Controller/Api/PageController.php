...
use Sonata\PageBundle\Controller\Api\PageController as BaseController;
...
//extends BaseController
{
     ...
     * @ApiDoc(
     *  requirements={
     *      {"name"="slug", "dataType"="string", "requirement"="\w+", "description"="page slug"}
     *  },
     *  output={"class"="Sonata\PageBundle\Model\PageInterface", "groups"="sonata_api_read"},
     *  statusCodes={
     *      200="Returned when successful",
     ...
     *
     * @View(serializerGroups="sonata_api_read", serializerEnableMaxDepthChecks=true)
     * @Get("/{slug}", name="_page_slug")
     *
     * @param $slug
     *
     * @return PageInterface
     */
    protected function getPageSlugAction($slug)
    {
        $page = $this->pageManager->findOneBy(array('slug' => $slug));
        if (null === $page) {
            throw new NotFoundHttpException(sprintf('Page (%d) not found', $slug));
        }
    ...
    {
        return $this->getPageSlugAction($id);
