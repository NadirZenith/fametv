...
class PostController
{
     ...
     */
    public function __construct(PostManagerInterface $postManager, CommentManagerInterface $commentManager, MailerInterface $mailer, FormFactoryInterface $formFactory, FormatterPool $formatterPool, CollectionManagerInterface $collectionManager)
    {
     ...
     * @QueryParam(name="count", requirements="\d+", default="12", description="Number of posts by page")
     ...
     * @QueryParam(name="collection", requirements="\S+", nullable=true, description="Posts by collection filter")
     * @QueryParam(name="search", requirements="\S+", nullable=true, description="Search filter")
     *
    ...
    public function getPostsAction(ParamFetcherInterface $paramFetcher)
    {
        ...
        if ($paramFetcher->get('search')) {
            return $this->getPostsSearchPager($this->filterCriteria($paramFetcher), $page, $count);
        }
